...
def evaluate(model, data_loader, criterion, device, save_output=False):
    ...
    with torch.no_grad():
        for i, (data) in tqdm(enumerate(data_loader), total=len(data_loader)):
            ...
            logit = logit[:, :target.size(
                1), :]  # cut over length to calculate loss
            loss = criterion(logit.contiguous().view(-1, logit.size(-1)),
                             target.contiguous().view(-1))
            total_loss += loss.item()
            total_num += sum(feat_lengths).item()
...
def main():
    ...
    parser.add_argument('--force-save', default=False, help="force to save")
    ...
    if args.mode != "train":
        ...
        for epoch in range(begin_epoch, args.epochs):
            ...
            if best_cer > cer_list[0] or args.force_save:
